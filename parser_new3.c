...
static void adv(Parser *p) { printf("[adv] call\n"); fflush(0); lexer_next(p->lex); printf("[adv] ret\n"); fflush(0); }
static int  adv_kind(Parser *p) { adv(p); return (int)p->lex->cur.kind; }
static int    chk(Parser *p, TokenKind k) { printf("[chk] k=%d p=%p\n",(int)k,(void*)p); fflush(0); return lexer_check(p->lex, k); }
static void eat(Parser *p, TokenKind k) { printf("[eat] call k=%d\n",(int)k); fflush(0); lexer_expect_void(p->lex, k); printf("[eat] ret k=%d\n",(int)k); fflush(0); }
...
TypeInfo *ParseTypeSpecifier(Parser *p) {
    ...
    if (k==TOK_IDENT) {
        printf("[pts] ident=%s p->sym=%p\n", tok_ident(&cur(p)), (void*)p->sym); fflush(0);
        Symbol *s = symtable_lookup(p->sym, tok_ident(&cur(p)));
        printf("[pts] lookup done s=%p\n", (void*)s); fflush(0);
...
ASTNode *ParseNumber(Parser *p) {
    if (cur(p).kind==TOK_FLOAT) { double fv=cur(p).fval; int ln=cur(p).line; adv(p); return ast_float(fv, ln); }
...
ASTNode *ParseIdentifier(Parser *p) {
    ...
    int t_line = cur(p).line;
    eat(p, TOK_IDENT);
    printf("[pid] post-eat name=%s\n",name); fflush(0);
    /* function call? */
    if (chk(p,TOK_LPAREN)) {
        adv(p);
        ...
        return ast_call(name, args, argc, t_line);
    }
    return ast_var(name, t_line);
}
...
ASTNode *ParsePrimary(Parser *p) {
    ...
    if (chk(p,TOK_STRING)) {
        ...
        char buf[4096]; snprintf(buf,sizeof buf,"%s",cur(p).sval?cur(p).sval:""); adv(p);
...
ASTNode *ParsePostfix(Parser *p) {
    printf("[ppfx] enter p=%p\n",(void*)p); fflush(0);
    ASTNode *n = ParsePrimary(p);
    printf("[ppfx] post-primary n=%p\n",(void*)n); fflush(0);
    for (;;) {
        int line = cur(p).line;
        if (chk(p,TOK_LBRACKET)) {
            adv(p);
...
ASTNode *ParseUnary(Parser *p) {
    ...
    printf("[pun] pre-pfix p=%p\n",(void*)p); fflush(0);
    ASTNode *rr = ParsePostfix(p);
    printf("[pun] post-pfix rr=%p\n",(void*)rr); fflush(0);
    return rr;
}
...
ASTNode *ParseStatement(Parser *p) {
    ...
    if (chk(p,TOK_IDENT)) {
        ...
        char nm[256]; strncpy(nm,tok_ident(&cur(p)),sizeof nm-1);
        adv(p);
        if (chk(p,TOK_COLON)) {
            adv(p);
...
ASTNode *ParseVariable(Parser *p) {
    ...
    /* Sync array_size back into type so symtable_define_var allocates correctly */
    ...
    if (chk(p, TOK_COMMA)) {
        ...
        while (chk(p, TOK_COMMA) && nd<63) {
            ...
            t2->base         = type->base;
            t2->pointer_depth= type->pointer_depth;
            t2->is_const     = type->is_const;
            t2->is_unsigned  = type->is_unsigned;
            t2->array_size   = type->array_size;
            t2->pointed_to   = type->pointed_to;
            t2->is_volatile  = type->is_volatile;
            t2->is_inline    = type->is_inline;
            t2->is_extern    = type->is_extern;
            t2->is_float     = type->is_float;
            ...
            t2->array_size = arr2; /* sync parsed array size into TypeInfo */
...
ASTNode *parse_program(Parser *p) {
    printf("[pg] enter parse_program tok=%d\n",(int)cur(p).kind); fflush(0);
    ASTNode *decls[1024]; int count=0;
    while (!chk(p,TOK_EOF)) {
        { const char *kn = token_kind_name(cur(p).kind); printf("[pg] loop tok=%d(%s) kind0=%d\n",(int)cur(p).kind,kn?kn:"?",(int)p->lex->cur.kind); fflush(0); }
        int line=cur(p).line;
        ...
        if (chk(p,TOK_TYPEDEF)) {
            adv(p);
            printf("[pg] typedef base tok=%d(%s)\n",(int)cur(p).kind,token_kind_name(cur(p).kind)); fflush(0);
            TypeInfo *ti = ParseTypeSpecifier(p);
            while (chk(p,TOK_STAR)) { adv(p); ti->pointer_depth++; }
            if (!chk(p,TOK_IDENT)) { parse_error(p,"expected typedef name"); eat(p,TOK_SEMICOLON); continue; }
            printf("[pg] alias pre: kind=%d len=%d start=%p\n",(int)cur(p).kind,cur(p).len,(void*)cur(p).start); fflush(0);
            char alias[256]; strncpy(alias,tok_ident(&cur(p)),sizeof alias-1); adv(p);
            eat(p,TOK_SEMICOLON);
            printf("[pg] typedef alias=%s\n",alias); fflush(0);
            symtable_define_typedef(p->sym, alias, ti);
